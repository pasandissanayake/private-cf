            ...
            for db_combination in db_combinations:
                ...
                for mu_combination in itertools.product(range(d_min), repeat=M):
                    ...
                    if noisy_combination in counts_x.keys():
                        ...
                        counts_x[noisy_combination] = 1
            count_logs.append(np.log(len(counts_x.keys())))

        max_leakage = (np.max(count_logs) - M*np.log(d_min)) / np.log(q)

        logger.log_info(f'max leakage={max_leakage} to the base {q}')
        max_leakages.append(max_leakage)
    return max_leakages
